...
`sync.php` is the entry point. It parses flags, builds `Config` from `config.php`, wires up the reconcilers, discovers the repo list, then for each repo runs every reconciler and prints a per-repo change report. Exit code is non-zero if any repo failed.

- **`config.php`** is the source of truth. Three layers that merge in order: `defaults` (every non-archived repo) → matching `groups` (named subsets) → `repos` (per-repo overrides). `files` and `settings` merge key-by-key, so a later layer *adds to* rather than replaces an earlier one. A file source of `false` opts a repo out of a file an earlier layer set (it still gets the settings). Managed-file sources are infra's own files, read from the repo root, so the controller dogfoods exactly what it distributes. A file source can also be a **closure** `(GitHub $gh, string $owner, string $repo): ?string` that computes the content from live repo state, returning `null` to skip the file for that repo. `.github/dependabot.yml` uses this (see `src/Dependabot.php`) to add a composer updater only when `composer.lock` is committed and a github-actions updater only when the repo has workflows. `composer.json` and `composer.lock` use it too (see `src/ComposerPolicy.php`) to align the repo's PHP policy with `maho`: it fetches the live files, edits them in place via Composer's `JsonManipulator` (preserving formatting so the diff is minimal), and returns `null` when the file is absent or nothing drifted. The two are one object with a `json()` and a `lock()` closure rather than two calls, because `content-hash` covers `require-dev`: a lock computed from different arguments than its manifest would be wrong. The lock's hash is a pure function of the manifest (`Locker::getContentHash()`), so nothing is resolved and no package version moves. The CI workflows use it too (see `src/CiMatrix.php`): `normalize()` rewrites a flow-style matrix (`phpstan.yml`, `syntax-php.yml`, `install-with-prefix.yml`) to `maho`'s version set, and `pin()` rewrites a single `php-version:` value (`lint.yml`) to the floor. Both use a surgical regex and return `null` when the workflow is absent or already aligned, so a file is never created, only fixed. Only workflows every repo shares belong here; a workflow one repo alone has is that repo's own to pin.
